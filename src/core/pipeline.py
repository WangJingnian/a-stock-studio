...
class StockAnalysisPipeline:
    ...
    def analyze_stock(
        ...
        try:
            ...
            if result:
                ...
                fill_cci_if_needed(result, trend_result)
                # 以现价为锚校正狙击点位/持仓建议，避免 LLM 机械引用均线导致
                # “买点远低于现价、止盈过低、止损表述矛盾”等不自洽输出
                normalize_battle_plan_prices(result, trend_result)
                # CCI 指标历史适用性回测（东财口径：上穿-100买/下穿+100卖）
                try:
                    attach_cci_fitness(result, self.fetcher_manager, self.db, code)
                except Exception as _ce:
                    logger.warning("[cci_fitness] %s attach error: %s", code, _ce)
    ...
    def _enhance_context(
        ...
        if trend_result:
            enhanced['trend_analysis'] = {
                ...
                'cci': trend_result.cci,
                'cci_prev': trend_result.cci_prev,
                'cci_status': trend_result.cci_status.value,
                'cci_signal': trend_result.cci_signal,
                'cci_cross_up': trend_result.cci_cross_up,
                'cci_cross_down': trend_result.cci_cross_down,
                'cci_cross_oversold': trend_result.cci_cross_oversold,
                'cci_signal_type': trend_result.cci_signal_type,
                'cci_signal_label': trend_result.cci_signal_label,
                'cci_signal_date': trend_result.cci_signal_date,
                'cci_signal_price': trend_result.cci_signal_price,
                'cci_signal_trigger': trend_result.cci_signal_trigger,
                'cci_live_type': trend_result.cci_live_type,
                'cci_live_label': trend_result.cci_live_label,
            }
    ...
    def _augment_historical_with_realtime(
        ...
        if market and not is_market_open(market, market_today):
            return df
        # 仅盘中补实时 K 线：is_market_open 只判断"当天是否交易日"，
        # 凌晨/盘后仍会返回 True，此时用昨收价伪造"今日"K 线会导致
        # 技术指标（如 CCI）基于虚拟行计算而失真。非交易时段直接跳过。
        try:
            from datetime import datetime as _dt
            _now = _dt.now()
            _hm = _now.hour * 60 + _now.minute
            _in_cn_session = (9 * 60 + 30 <= _hm <= 11 * 60 + 30) or (13 * 60 <= _hm <= 15 * 60)
            if market == 'cn' and not _in_cn_session:
                return df
        except Exception:
            pass
