...
class CCIStatus(Enum):
    """CCI（顺势指标）状态枚举"""
    EXTREME_OVERBOUGHT = "极端超买"   # CCI > +200，超常波动，防反转
    OVERBOUGHT = "超买"              # CCI > +100
    STRONG = "强势"                  # 0 < CCI <= +100，多方占优
    NEUTRAL = "中性"                 # CCI ≈ 0，常态区间
    WEAK = "弱势"                    # -100 <= CCI < 0，空方占优
    OVERSOLD = "超卖"                # CCI < -100，关注反弹
...
class TrendAnalysisResult:
    ...
    # CCI 指标（顺势指标）
    cci: float = 0.0                 # CCI(14)
    cci_prev: float = 0.0            # 前一交易日 CCI，用于穿越判断
    cci_status: CCIStatus = CCIStatus.NEUTRAL
    cci_signal: str = ""             # CCI 信号描述
    cci_cross_up: bool = False       # 上穿 +100（强势启动）
    cci_cross_down: bool = False     # 跌破 +100（上涨动能减弱）
    cci_cross_oversold: bool = False # 上穿 -100（超卖反转买入信号）

    # CCI 直白信号（实时：基于最新两根 K 线穿越）
    cci_live_type: str = "watch"      # buy / sell / watch
    cci_live_label: str = "观望"      # 买入信号 / 卖出信号 / 观望

    # CCI 确认信号（最近一次历史穿越，带日期与触发价）
    cci_signal_type: str = "watch"     # buy / sell / watch
    cci_signal_label: str = "观望"     # 买入信号 / 卖出信号 / 观望
    cci_signal_date: str = ""          # 最近一次确认信号的日期（YYYY-MM-DD）
    cci_signal_price: float = 0.0      # 信号触发日收盘价（元）
    cci_signal_trigger: str = ""       # 触发原因描述（如上穿-100超卖反转）
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            'cci': self.cci,
            'cci_prev': self.cci_prev,
            'cci_status': self.cci_status.value,
            'cci_signal': self.cci_signal,
            'cci_cross_up': self.cci_cross_up,
            'cci_cross_down': self.cci_cross_down,
            'cci_cross_oversold': self.cci_cross_oversold,
            'cci_signal_type': self.cci_signal_type,
            'cci_signal_label': self.cci_signal_label,
            'cci_signal_date': self.cci_signal_date,
            'cci_signal_price': self.cci_signal_price,
            'cci_signal_trigger': self.cci_signal_trigger,
            'cci_live_type': self.cci_live_type,
            'cci_live_label': self.cci_live_label,
        }
...
class StockTrendAnalyzer:
    ...
    # CCI（顺势指标）参数
    CCI_PERIOD = 14             # 默认周期（与主流软件一致）
    CCI_OVERBOUGHT = 100        # 超买线 +100
    CCI_OVERSOLD = -100         # 超卖线 -100
    CCI_EXTREME = 200           # 极端阈值 ±200
    CCI_CONSTANT = 0.015        # 归一化常数（CCI 标准公式）
    ...
    def _calculate_cci(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        公式（与主流软件一致）：
        - 典型价 TYP = (最高 + 最低 + 收盘) / 3
        - 平均偏差 MD = N 日内 |TYP - TYP均值| 的均值
        - CCI = (TYP - TYP的N日均值) / (0.015 × MD)

        CCI 无上下限，通过 +100 / -100（极端 ±200）判断超买超卖；
        对价格波动敏感，突破/跌破 -100、+100 常作为买卖信号。
        """
        df = df.copy()

        if not {'high', 'low', 'close'}.issubset(df.columns):
            logger.warning("缺少 high/low/close 列，跳过 CCI 计算")
            df['CCI'] = 0.0
            return df

        typ = (df['high'] + df['low'] + df['close']) / 3.0
        ma_typ = typ.rolling(window=self.CCI_PERIOD, min_periods=self.CCI_PERIOD).mean()

        # 平均偏差：窗口内 |TYP - TYP窗口均值| 的均值
        mean_dev = typ.rolling(
            window=self.CCI_PERIOD,
            min_periods=self.CCI_PERIOD,
        ).apply(
        ...
        # 防止分母为 0
        safe_dev = mean_dev.replace(0, np.nan)
        cci = (typ - ma_typ) / (self.CCI_CONSTANT * safe_dev)

        df['CCI'] = cci.fillna(0.0).round(2)
        return df
    ...
    def _analyze_cci(self, df: pd.DataFrame, result: TrendAnalysisResult) -> None:
        """
        分析 CCI（顺势指标）

        核心判断（与东财等主流软件口径一致）：
        - CCI 上穿 +100：进入超买/强势区，非买卖信号（与东财口径一致）
        - CCI 下穿 +100：超买回落，经典卖出信号
        - CCI 上穿 -100：超卖反转，经典买入信号
        - CCI < -100：超卖区，关注反弹
        - CCI > +200 / < -200：极端波动，防趋势反转
        - 0 附近：常态区间，趋势不明
        """
        if len(df) < self.CCI_PERIOD + 1:
            result.cci_signal = "数据不足"
            return

        latest = df.iloc[-1]
        prev = df.iloc[-2]
        result.cci = float(latest['CCI'])
        result.cci_prev = float(prev['CCI'])

        cci = result.cci
        prev_cci = result.cci_prev
        overbought = self.CCI_OVERBOUGHT
        oversold = self.CCI_OVERSOLD
        extreme = self.CCI_EXTREME

        # 穿越判断（基于相邻两日）
        result.cci_cross_up = prev_cci <= overbought < cci
        result.cci_cross_down = prev_cci >= overbought > cci
        result.cci_cross_oversold = prev_cci <= oversold < cci

        # 状态与信号文本
        if cci > extreme:
            result.cci_status = CCIStatus.EXTREME_OVERBOUGHT
            result.cci_signal = f"⚠️ CCI极端超买({cci:.1f}>200)，超常波动，谨防趋势反转"
        elif cci > overbought:
            if result.cci_cross_up:
                result.cci_status = CCIStatus.OVERBOUGHT
                result.cci_signal = f"⬆️ CCI上穿+100({cci:.1f})，进入强势/超买区，关注后续动能"
            else:
                result.cci_status = CCIStatus.OVERBOUGHT
                result.cci_signal = f"⚠️ CCI超买({cci:.1f}>100)，短线防回调"
        elif cci > 0:
            result.cci_status = CCIStatus.STRONG
            result.cci_signal = f"⚡ CCI强势区({cci:.1f})，多方占优"
        elif cci >= oversold:
            result.cci_status = CCIStatus.WEAK
            result.cci_signal = f" CCI弱势区({cci:.1f})，空方占优，观望为主"
        else:
            if result.cci_cross_oversold:
                result.cci_status = CCIStatus.OVERSOLD
                result.cci_signal = f"✅ CCI上穿-100({cci:.1f})，超卖反转，经典买入信号"
            else:
                result.cci_status = CCIStatus.OVERSOLD
                result.cci_signal = f"⭐ CCI超卖({cci:.1f}<-100)，关注反弹机会"

        # 下穿 +100 的信号补充（超买回落，卖出信号提示）
        if result.cci_cross_down:
            result.cci_signal = f"{result.cci_signal}；⚠️ 且 CCI 下穿+100，超买回落，注意卖出信号"

        # 直白信号类型（实时：基于最新两根 K 线的穿越）
        # 与东财/通达信 CCI 箭头口径一致：买入=上穿-100（超卖反转）；卖出=下穿+100（超买回落）
        if result.cci_cross_oversold:
            result.cci_live_type = "buy"
            result.cci_live_label = "买入信号"
        elif result.cci_cross_down:
            result.cci_live_type = "sell"
            result.cci_live_label = "卖出信号"
        else:
            result.cci_live_type = "watch"
            result.cci_live_label = "观望"

        # 最近一次确认信号（带日期与触发日收盘价），只填确认信号字段，不覆盖实时信号
        self._find_latest_cci_signal(df, result)

    def _find_latest_cci_signal(self, df: pd.DataFrame, result: TrendAnalysisResult) -> None:
        """
        从最新一根 K 线往前扫描，定位最近一次 CCI 穿越信号，记录触发日期与当日收盘价。
        ...
        for i in range(len(df) - 1, 0, -1):
            prev_val, cur_val = cci_arr[i - 1], cci_arr[i]
            # 买入：上穿 -100（超卖反转，经典抄底）
            if prev_val <= oversold < cur_val:
                result.cci_signal_type = "buy"
                result.cci_signal_label = "买入信号"
                result.cci_signal_date = str(dates[i])
                result.cci_signal_price = float(closes[i])
                result.cci_signal_trigger = "上穿-100 · 超卖反转"
                return
            # 卖出：下穿 +100（超买回落）
            if prev_val >= overbought > cur_val:
                result.cci_signal_type = "sell"
                result.cci_signal_label = "卖出信号"
                result.cci_signal_date = str(dates[i])
                result.cci_signal_price = float(closes[i])
                result.cci_signal_trigger = "下穿+100 · 超买回落"
                return
    ...
    def _generate_signal(self, result: TrendAnalysisResult) -> None:
        ...
        # === RSI 评分（5分，为 CCI 腾出 5 分空间，总分保持 100）===
        rsi_scores = {
            RSIStatus.OVERSOLD: 5,       # 超卖最佳
            RSIStatus.STRONG_BUY: 4,     # 强势
            RSIStatus.NEUTRAL: 2,        # 中性
            RSIStatus.WEAK: 1,            # 弱势
            RSIStatus.OVERBOUGHT: 0,       # 超买最差
        }
        rsi_score = rsi_scores.get(result.rsi_status, 2)
        score += rsi_score
        ...
        # === CCI 评分（5分）===
        # 穿越信号优先：上穿 -100（超卖反转抄底）最强，上穿 +100（强势启动）次之
        if result.cci_cross_oversold:
            cci_score = 5
            reasons.append(f"✅ {result.cci_signal}")
        elif result.cci_cross_up:
            cci_score = 4
            reasons.append(f"✅ {result.cci_signal}")
        else:
            cci_scores = {
                CCIStatus.OVERSOLD: 4,        # 超卖区，关注反弹
                CCIStatus.STRONG: 3,          # 强势区
                CCIStatus.NEUTRAL: 2,         # 中性
                CCIStatus.WEAK: 1,            # 弱势区
                CCIStatus.OVERBOUGHT: 1,      # 高位超买，防回调
                CCIStatus.EXTREME_OVERBOUGHT: 0,  # 极端超买最差
            }
            cci_score = cci_scores.get(result.cci_status, 2)
            if result.cci_status in (CCIStatus.OVERSOLD, CCIStatus.STRONG):
                reasons.append(f"✅ {result.cci_signal}")
            elif result.cci_status in (CCIStatus.OVERBOUGHT, CCIStatus.EXTREME_OVERBOUGHT):
                risks.append(f"⚠️ {result.cci_signal}")
            else:
                reasons.append(result.cci_signal)
        score += cci_score
    ...
    def format_analysis(self, result: TrendAnalysisResult) -> str:
        ...
        lines = [
            ...
            f"",
            f"📊 CCI指标: {result.cci_status.value}",
            f"   CCI(14): {result.cci:.1f}",
            f"   上穿+100: {'是' if result.cci_cross_up else '否'} | 跌破+100: {'是' if result.cci_cross_down else '否'} | 上穿-100: {'是' if result.cci_cross_oversold else '否'}",
            f"   信号: {result.cci_signal}",
            f"",
