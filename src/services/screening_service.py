# -*- coding: utf-8 -*-
"""SEA-native stock screening service.
...
class ScreeningService:
    """Coordinate stock screening with SEA-owned capabilities."""
        ...
        *,
        ...
        if isinstance(provider_arg, DsaEastMoneyHotspotProvider) and _hotspot_rows_are_thin(cache_rows, top=cache_top_count):
            try:
                ...
                logger.warning("Screening SEA direct hotspot fallback failed: %s", exc)
                direct_hotspots = []
            ...
            if len(direct_hotspots) > len(cache_rows):
                ...
                source_errors.append("Screening hotspot rows were thin; used SEA EastMoney board-change rows.")
...
def _build_screening_dsa_daily_history_fetcher() -> Optional[Callable[..., Any]]:
    """Build one request-local SEA-first daily-history fetcher.
    ...
    def fetch_daily_history_with_dsa(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                "Screening SEA daily history fetch failed for %s; falling back to Screening source %s: %s",
                code,
                source,
                exc,
            )
...
class DsaEastMoneyHotspotProvider:
    ...
    def _fetch_constituent_sources(self, topic: str, *, source: str) -> List[Any]:
        ...
        AkShare calls run in SEA's killable timeout subprocess; direct HTTP
...
def _fetch_dsa_hotspot_rankings(source: str, limit: int) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """Load SEA's ranking fallback inside the caller's killable subprocess."""
...
def search_dsa_stock_news(stock_code: str, stock_name: str = "", max_results: int = 3) -> Dict[str, Any]:
    ...
    if not getattr(service, "is_available", False):
        return {
            "success": False,
            "error": "SEA search service unavailable",
            "results": [],
        }
...
def search_dsa_stock_events(stock_code: str, stock_name: str = "", max_results: int = 3) -> Dict[str, Any]:
    """Reuse SEA event search for earnings, reduction and announcement context."""
    service = _get_dsa_search_service()
    if not getattr(service, "is_available", False):
        return {
            "success": False,
...
def _enrich_candidates_with_dsa(candidates: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    for index, candidate in enumerate(candidates):
        ...
        except Exception as exc:  # noqa: BLE001 - SEA enrichment must not block screening.
            code = candidate.get("code") or f"rank-{candidate.get('rank', index + 1)}"
            message = f"{code}: {exc}"
            warnings.append(message)
            logger.warning("SEA enrichment failed for Screening candidate %s: %s", code, exc)
...
def _build_dsa_analysis_summary(
    ...
    if isinstance(coverage, dict) and coverage:
        ...
        if available_blocks:
            parts.append(f"SEA基本面覆盖：{', '.join(available_blocks[:4])}")
    ...
    if isinstance(news_results, list) and news_results:
        ...
        if titles:
            parts.append(f"SEA新闻：{'；'.join(titles[:2])}")
    ...
    if isinstance(event_results, list) and event_results:
        ...
        titles = [title for title in titles if title]
        if titles:
            parts.append(f"SEA事件：{'；'.join(titles[:2])}")
