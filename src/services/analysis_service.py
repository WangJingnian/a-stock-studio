...
def _extract_cci_strategy(result) -> Dict[str, Any]:
    """
    从 result.dashboard.data_perspective.trend_status 提取 CCI 指标点位字段。

    CCI 由趋势分析引擎计算并写入 dashboard（fill_cci_if_needed），这里把它
    附带到报告 strategy 上，供前端在“策略点位”下方展示 CCI 指标点位。
    """
    try:
        ...
        if not isinstance(trend_status, dict):
            return {}
        ...
        logger.warning("[cci_strategy] extract failed, skipping: %s", e)
        return {}
