...
"""SEA provider-context bridge.

This module consumes SEA-owned callables passed through ``context["dsa"]``.
It is intentionally best-effort: the screening engine can use richer SEA data when
...
def apply_dsa_provider_context(
    ...
    *,
    ...
    """Attach SEA context to top candidates before LLM ranking."""
    ...
    for pick in picks[:limit]:
        try:
            ...
            logger.warning("SEA provider context failed for %s: %s", pick.code, exc)

    notes = [f"SEA provider context applied {enriched_count} of {limit} candidates"]
    if errors:
        sample = " | ".join(errors[:5])
        suffix = f" | +{len(errors) - 5} more" if len(errors) > 5 else ""
        notes.append(f"SEA provider context row errors: {sample}{suffix}")
    return notes
...
def _build_dsa_summary(pick: Pick, context: dict[str, Any], news: list[dict[str, Any]]) -> str:
    ...
    if isinstance(coverage, dict):
        ...
        if available:
            parts.append(f"SEA基本面覆盖: {', '.join(available[:4])}")
