...
def fetch_daily_history(
    ...
    *,
    ...
    if src == "auto":
        ...
        # 场内 ETF（5 开头沪 / 159 开头深）价格为 3 位小数，腾讯源常返回 2 位近似，
        # 优先使用 3 位精度的源（sina/tushare），保证市值与账本一致
        if _is_traded_etf_code(normalized_code):
            preferred = [s for s in ("sina",) if s in sources]
            sources = tuple(preferred + [s for s in sources if s != "sina"])
...
def _is_traded_etf_code(code: str) -> bool:
    """判断是否为场内 ETF（沪 5 开头 / 深 159 开头）。"""
    c = (code or "").strip()
    return c.startswith("5") or c.startswith("159")
