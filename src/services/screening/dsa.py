...
"""Optional SEA integration for L3 deep analysis."""
...
def analyze_picks_with_dsa(
    ...
    *,
    ...
    """Run SEA deep analysis for up to ``max_picks`` candidates in place.
    ...
    for idx, pick in enumerate(picks):
        ...
        try:
            ...
            logger.warning("SEA deep analysis failed for %s: %s", pick.code, exc)
            pick.deep_analysis_status = "failed"
            pick.deep_analysis_error = str(exc)
            degradation.append(f"SEA deep analysis failed for {pick.code}: {exc}")
...
def check_dsa_readiness(api_url: str, *, timeout_sec: float = 5.0) -> dict:
    """Best-effort SEA endpoint readiness probe for CLI/runtime diagnostics."""
    if not api_url:
        return {
            "available": False,
            "status": "missing_url",
...
def call_dsa_analysis(
    ...
    *,
    ...
    """Call the SEA sync analysis endpoint and return parsed JSON."""
    payload = {
    ...
    if query_id:
        # The current SEA public API does not require query_id,
...
def extract_deep_analysis_summary(result: dict) -> str:
    """Best-effort extraction of a short summary from a SEA response."""
...
def apply_dsa_overlay(picks: list[Pick]) -> list[Pick]:
    """Use SEA structured output as the final low-frequency risk/alpha overlay."""
