...
def _build_fundamentals_block(artifacts: PipelineAnalysisArtifacts) -> AnalysisContextBlock:
    ...
    # 核心财务模块（估值/成长/盈利）只要有真实数据即视为基本面可用；
    # 资金流/龙虎榜/板块等辅助模块失败不应再把整个基本面降级为“部分可用”。
    core_statuses = [
        str(coverage.get(block, "")).lower()
        for block in ("valuation", "growth", "earnings")
    ]
    core_has_data = any(
        isinstance(context.get(block), dict) and bool(context[block].get("data"))
        for block in ("valuation", "growth", "earnings")
    )
    if core_has_data:
        status = ContextFieldStatus.AVAILABLE
