...
class HistoryService:
    ...
    def get_news_intel(self, query_id: str, limit: int = 20) -> List[Dict[str, str]]:
        ...
        try:
            ...
            # 库内无资讯时，实时用免费搜索源抓取一次并落库，避免历史报告长期"暂无相关资讯"
            if not items:
                items = self._live_search_news_fallback(query_id=query_id, limit=limit)
    ...
    def _live_search_news_fallback(self, query_id: str, limit: int) -> List[Dict[str, str]]:
        """库内无该次分析资讯时，实时抓取一次（东财公告+新闻免费接口优先，search 服务兜底）。"""
        try:
            records = self.db.get_analysis_history(query_id=query_id, limit=1)
            if not records or not records[0].code:
                return []
            record = records[0]
            stock_name = record.name or record.code

            # 优先东财免费接口（不消耗 LLM 搜索额度）
            em_items = self._live_news_from_eastmoney(record.code, stock_name, limit)
            if em_items:
                return em_items

            # 新浪财经兜底（东财不可用时不依赖 LLM）
            sina_items = self._live_news_from_sina(record.code, stock_name, limit)
            if sina_items:
                return sina_items
            ...
            logger.warning("实时新闻兜底失败 (%s): %s", query_id, e)
            return []

    @staticmethod
    def _live_news_from_eastmoney(code: str, stock_name: str, limit: int) -> List[Dict[str, str]]:
        """东财免费公告 + 新闻搜索接口，实时抓取个股资讯（不依赖 LLM）。"""
        ...
        try:
            ann_url = (
                ...
                f"?sr=-1&page_size={max(limit, 8)}&page_index=1&ann_type=A"
                f"&client_source=web&stock_list={code}"
            )
            data = _json.loads(_fetch(ann_url, "https://data.eastmoney.com/notices/"))
            for a in (data.get("data") or {}).get("list") or []:
                ...
                items.append({
                    "title": f"[公告] {title}",
                    "snippet": f"公告日期 {date}" if date else "",
                    "url": url,
                })
        except Exception:
            pass
        ...
        try:
            param = _json.dumps({
                ...
                "param": {"cmsArticleWebOld": {
                    "searchScope": "default", "sort": "time", "pageIndex": 1,
                    "pageSize": max(limit, 8), "preTag": "", "postTag": "",
                }},
            }, ensure_ascii=False)
            search_url = (
            ...
            body = _fetch(search_url, "https://so.eastmoney.com/")
            m = re.search(r"\((\{.*\})\)\s*$", body, re.S)
            if m:
                data = _json.loads(m.group(1))
                for a in (data.get("result") or {}).get("cmsArticleWebOld") or []:
                    title = (a.get("title") or "").replace("<em>", "").replace("</em>", "").strip()
                    if not title:
                        continue
                    ...
                    items.append({
                        "title": f"[新闻] {title}",
                        "snippet": f"发布时间 {date}" if date else "",
                        "url": url,
                    })
        except Exception:
            pass
        ...
        for it in items:
            ...
            if len(snippet) > 200:
                snippet = f"{snippet[:197]}..."
            out.append({"title": it["title"], "snippet": snippet, "url": it.get("url") or ""})
            if len(out) >= limit:
                break
        return out

    @staticmethod
    def _live_news_from_sina(code: str, stock_name: str, limit: int) -> List[Dict[str, str]]:
        """新浪财经个股新闻兜底（东财不可用且不依赖 LLM 时使用）。

        直接解析新浪官方个股"相关资讯"列表页（vip.stock.finance.sina.com.cn），
        内容与个股强相关（公告/互动/新闻），比通用资讯流更精准。
        """
        import re as _re
        import urllib.request
        ...
        items: List[Dict[str, str]] = []
        seen: set = set()
        # 股票名核心词（去常见后缀），用于过滤无关新闻
        core = stock_name or ""
        for _suffix in ("ETF", "股份", "有限", "公司", "集团", "科技", "电子", "控股", "证券"):
            core = core.replace(_suffix, "")
        core = (core or "").strip()
        # 提取 datelist 区块里的 日期 + 链接 + 标题
        for m in _re.finditer(
            r"(\d{4}-\d{2}-\d{2})[^<]*<a[^>]*href=['\"]([^'\"]+)['\"][^>]*>([^<]{4,120})</a>",
            body,
        ):
            date, url_href, title = m.group(1), m.group(2), m.group(3).strip()
            title = _re.sub(r"\s+", " ", title)
            if not title or title in seen:
                continue
            # 标题不含股票名核心词则跳过（避免 ETF 等无个股资讯时混入无关新闻）
            if core and core not in title:
                continue
            seen.add(title)
            items.append({
                "title": f"[新闻] {title}",
                "snippet": f"发布时间 {date}",
                "url": url_href if url_href.startswith("http") else f"https:{url_href}",
            })
    ...
    def _generate_single_stock_markdown(
        ...
        if data_persp:
            ...
            if trend_data:
                ...
                # CCI(14) 顺势指标
                cci_val = trend_data.get('cci')
                if cci_val not in (None, ''):
                    cci_status = trend_data.get('cci_status', '')
                    cci_signal = trend_data.get('cci_signal', '')
                    report_lines.extend([
                        f"**CCI(14)**: {cci_val} | 状态: {cci_status} | 信号: {cci_signal}",
                        "",
                    ])
            # 价格位置
            if price_data:
