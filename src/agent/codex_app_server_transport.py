...
all SEA tools are supplied through the Phase 6a :class:`ToolSurface` boundary.
"""
...
_BASE_CONFIG_OVERRIDES = (
    ...
    'permissions.dsa_gate_a.description="SEA Agent read-only workspace"',
...
def normalize_token_usage_notification(params: dict) -> Optional[dict]:
    """Map the documented App Server per-turn usage payload to SEA telemetry keys."""
...
def dynamic_tool_specs(surface: ToolSurface, names: Iterable[str]) -> list[dict]:
    ...
    for name in names:
        ...
        if descriptor is None:
            raise CodexAppServerError("tool_not_found", f"SEA tool is not registered: {name}")
        specs.append(
            {
                "type": "function",
    ...
    def close(self) -> None:
        ...
        if not tool_cleanup_ok:
            close_error = CodexAppServerError(
                ...
                "One or more SEA tool workers could not be fully reclaimed",
            )
        ...
        *,
        ...
        if active_profile.get("id") != PERMISSION_PROFILE:
            raise CodexAppServerError(
                ...
                "App Server did not activate the SEA permission profile",
            )
        with self._state_lock:
    ...
    def _execute_tool_request(self, message: dict) -> None:
        ...
        if error_code in {
            ...
            self._set_fatal(
                CodexAppServerError(
                    error_code,
                    "SEA tool process boundary failed",
                    turn_started=True,
                )
            )
            self._terminate_process()
