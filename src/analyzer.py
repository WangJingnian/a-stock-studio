...
_CCI_KEYS = ("cci", "cci_status", "cci_signal", "cci_cross_up", "cci_cross_down", "cci_cross_oversold",
             "cci_signal_type", "cci_signal_label", "cci_signal_date", "cci_signal_price", "cci_signal_trigger",
             "cci_live_type", "cci_live_label")


def fill_cci_if_needed(result: "AnalysisResult", trend_result: Any = None) -> None:
    """Fill CCI(14) fields into data_perspective.trend_status from trend analysis (in-place).

    This keeps the CCI indicator (value / status / signal / cross flags) visible in
    the persisted dashboard so both the report renderer and the share poster can
    surface it in the technical section.
    """
    if not result:
        return
    try:
        ...
        computed: Dict[str, Any] = {}
        for key in _CCI_KEYS:
            val = tr.get(key)
            if val is None:
                continue
            if key == "cci_status" and hasattr(val, "value"):
                val = val.value
            computed[key] = val
        if not computed:
            return

        # CCI 是纯技术指标计算值：LLM 生成 dashboard 时这些字段不可信（可能编造数值），
        # 一律用真实计算值覆盖，避免展示 LLM 幻觉的 CCI 数值/信号。
        filled = False
        ...
        if filled:
            dp["trend_status"] = ts
            logger.info("[trend_status] Filled CCI fields from trend analysis")
    except Exception as e:
        logger.warning("[cci] Fill failed, skipping: %s", e)


def normalize_battle_plan_prices(result: "AnalysisResult", trend_result: Any = None) -> None:
    """以现价为锚校正 battle_plan 狙击点位与持仓建议（in-place）。

    LLM 生成点位时机械引用均线数值，现价暴涨/暴跌后常出现
    “买点远低于现价、止盈过低、止损表述矛盾”等不自洽输出，
    这里用规则基于现价+均线重写四个点位及持仓建议表述，
    保证详情页与分享页展示的价格逻辑自洽、直白可读。
    """
    if not result:
        return
    try:
        ...
        price = _first_numeric_value(
            getattr(result, "current_price", None),
            tr.get("current_price"),
        )
        if not price or price <= 0:
            return
        ma5 = _first_numeric_value(tr.get("ma5")) or price
        ma20 = _first_numeric_value(tr.get("ma20")) or price
        recent_high = _first_numeric_value(tr.get("resistance_levels")) or round(price * 1.08, 2)
        ...
        if isinstance(battle, dict):
            ...
            if not isinstance(sp, dict):
                sp = {}
            bias = (price - ma5) / ma5 * 100 if ma5 else 0.0
            if bias > 1.5:
                ideal = round(max(ma5, price * 0.98), 2)
                ideal_note = "现价已高于5日线，回踩支撑企稳再看"
            else:
                ideal = round(price * 0.99, 2)
                ideal_note = "现价附近回踩企稳"
            secondary = round(min(ma20, price * 0.96), 2) if price > ma20 else round(price * 0.98, 2)
            if secondary >= ideal:
                secondary = round(ideal * 0.98, 2)
            stop = round(min(price * 0.95, (secondary or ideal) * 0.98), 2)
            stop = max(stop, round(price * 0.90, 2))
            take = round(max(price * 1.05, recent_high * 1.01), 2)

            sp["ideal_buy"] = f"{ideal:.2f}元（{ideal_note}）"
            sp["secondary_buy"] = f"{secondary:.2f}元（回踩20日线/现价下方分批）"
            stop_pct = int(round((price - stop) / price * 100))
            sp["stop_loss"] = f"{stop:.2f}元（现价下方约{stop_pct}%，跌破止损）"
            take_pct = int(round((take - price) / price * 100))
            sp["take_profit"] = f"{take:.2f}元（现价上方约{take_pct}%，突破看高）"
            battle["sniper_points"] = sp
            ...
            if isinstance(core, dict):
                ...
                if isinstance(pa, dict):
                    if price >= ma20:
                        pa["has_position"] = (
                            f"现价{price:.2f}位于20日线{ma20:.2f}上方，回踩不破{ma20:.2f}可继续持有；"
                            f"若放量跌破{ma20:.2f}需减仓控制风险。"
                        )
                        pa["no_position"] = (
                            f"现价{price:.2f}已高于5日线{ma5:.2f}，短线不宜追高，"
                            f"等回踩{ideal:.2f}附近企稳再考虑介入。"
                        )
                    else:
                        pa["has_position"] = (
                            f"现价{price:.2f}已跌破20日线{ma20:.2f}，反弹至{ma20:.2f}附近承压，"
                            f"建议逢反弹减仓控制风险。"
                        )
                        pa["no_position"] = (
                            f"现价{price:.2f}位于20日线{ma20:.2f}下方，趋势偏弱，"
                            f"建议观望，待企稳后再介入。"
                        )
            ...
            logger.info("[battle_plan] 已按现价 %.2f 校正狙击点位与持仓建议", price)
    except Exception as e:
        logger.warning("[battle_plan] normalize failed: %s", e)


def evaluate_cci_fitness(
    fetcher: Any = None,
    db: Any = None,
    code: str = "",
    lookback_days: int = 520,
) -> Optional[Dict[str, Any]]:
    """基于东财 CCI 口径对个股历史走势做信号回测，判断 CCI 指标是否适用于该股票。

    口径（与东财 K 线副图一致）：
    - 买入信号 = CROSS(CCI, -100)，即 CCI 上穿 -100（超卖反转）
    - 卖出信号 = CROSS(100, CCI)，即 CCI 下穿 +100（超买回落）
    以信号出现后的下一交易日开盘价成交（信号当日不成交，避免未来函数）。
    优先用 fetcher 拉约 520 个自然日（约 2 年）历史，失败时退回本地库。
    返回 {"signals","wins","win_rate","strategy_return","buy_hold_return","fitness","summary","cci_now"}。
    """
    if not code:
        return None
    df = None
    if fetcher is not None:
        try:
            df, _src = fetcher.get_daily_data(code, days=lookback_days)
    ...
    try:
        ...
        ma_typ = typ.rolling(14, min_periods=14).mean()
        mean_dev = typ.rolling(14, min_periods=14).apply(
            lambda v: float(np.mean(np.abs(v - np.mean(v)))), raw=True
        )
        cci = (typ - ma_typ) / (0.015 * mean_dev.replace(0, np.nan))
        df["CCI"] = cci.fillna(0.0)

        prev = df["CCI"].shift(1)
        buy_sig = (prev <= -100) & (df["CCI"] > -100)
        sell_sig = (prev >= 100) & (df["CCI"] < 100)
        ...
        entry_date = None
        for i in range(1, n):
            if buy_sig.iloc[i] and not position:
                position = True
                entry_price = float(opens[i])
                entry_date = dates[i]
            elif sell_sig.iloc[i] and position and entry_price:
                exit_price = float(opens[i])
                pnl = (exit_price - entry_price) / entry_price * 100
                trades.append((entry_date, dates[i], entry_price, exit_price, pnl))
                equity *= exit_price / entry_price
                position = False
        if position and entry_price:
            exit_price = float(closes[-1])
            pnl = (exit_price - entry_price) / entry_price * 100
            trades.append((entry_date, dates[-1], entry_price, exit_price, pnl))
            equity *= exit_price / entry_price
        ...
        buy_hold = (float(closes[-1]) / float(closes[0]) - 1) * 100

        if total < 2:
            fitness = "信号较少"
            summary = "近两年 CCI 买卖信号不足 2 次，指标参考意义有限。"
        elif win_rate >= 50 and strategy_return > 0:
            fitness = "适用"
            summary = (
                f"CCI 信号胜率 {win_rate:.0f}%，策略收益 {strategy_return:+.1f}%"
                f"（同期持有 {buy_hold:+.1f}%），适合用 CCI 辅助择时。"
            )
        elif win_rate >= 40 or strategy_return > buy_hold:
            fitness = "一般"
            summary = (
                f"CCI 信号胜率 {win_rate:.0f}%、策略收益 {strategy_return:+.1f}%"
                f"（同期持有 {buy_hold:+.1f}%），仅可作辅助参考。"
            )
        else:
            fitness = "不适用"
            summary = (
                f"CCI 信号胜率仅 {win_rate:.0f}%、策略收益 {strategy_return:+.1f}%"
                f"（同期持有 {buy_hold:+.1f}%），该股票用 CCI 信号难以盈利，不建议作为买卖依据。"
            )
        ...
        return {
            ...
            "strategy_return": round(strategy_return, 1),
            "buy_hold_return": round(buy_hold, 1),
            "fitness": fitness,
            "summary": summary,
            "cci_now": round(float(df["CCI"].iloc[-1]), 1),
        }
    except Exception as e:
        logger.warning("[cci_backtest] %s 回测失败: %s", code, e)
        return None


def attach_cci_fitness(result: "AnalysisResult", fetcher: Any = None, db: Any = None, code: str = "") -> None:
    """把 CCI 历史回测适用性评估写入 dashboard.data_perspective.trend_status.cci_fitness（in-place）。"""
    if not result or not code:
        return
    try:
        fitness = evaluate_cci_fitness(fetcher, db, code)
        if not fitness:
            return
        dash = result.dashboard if isinstance(result.dashboard, dict) else {}
        if not dash:
        ...
        if not isinstance(ts, dict):
            ts = {}
        ts["cci_fitness"] = fitness
        dp["trend_status"] = ts
        dash["data_perspective"] = dp
        result.dashboard = dash
        logger.info("[cci_fitness] %s 回测适用性: %s", code, fitness.get("fitness"))
    except Exception as e:
        logger.warning("[cci_fitness] %s attach failed: %s", code, e)
