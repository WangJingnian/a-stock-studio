...
class EastMoneySearchProvider(BaseSearchProvider):
    """东方财富个股资讯搜索（免费，无需真实 API Key）

    基于东方财富公开搜索接口（so.eastmoney.com 同源数据），直接检索个股
    新闻 / 公告 / 研报 / 快讯，无需任何付费 Key，作为 Anspire 等付费引擎
    额度耗尽或失败时的免费兜底数据源。

    说明：本类使用占位 Key（"free"）通过 BaseSearchProvider 的 key 检查，
    实际请求不携带也不依赖任何 API Key。
    """

    _BASE_URL = "https://search-api-web.eastmoney.com/search/jsonp"

    def __init__(self, api_keys: Optional[List[str]] = None):
        super().__init__(api_keys or ["free"], "东方财富")
    ...
    def _do_search(
        ...
        try:
            param = {
                ...
                "param": {
                    "cmsArticleWebOld": {
                        ...
                        "sort": "default",
                        "pageIndex": 1,
                        "pageSize": max(10, min(int(max_results) + 5, 50)),
                        "preTag": "",
                        "postTag": "",
                    }
                },
            ...
            match = _re.search(r"^[^(]*\((.*)\)\s*$", resp.text, _re.S)
            if not match:
                return SearchResponse(
                    query=query,
                    results=[],
            ...
            for item in articles[:max_results]:
                title = _re.sub(r"<[^>]+>", "", item.get("title", "") or "")
                content = _re.sub(r"<[^>]+>", "", item.get("content") or "")
                if len(content) > 400:
                    content = content[:400] + "..."
                results.append(
                    SearchResult(
                        ...
                        snippet=content,
                        url=item.get("url", "") or "",
                        source=item.get("mediaName") or self._extract_domain(item.get("url", "") or ""),
    ...
    def _extract_keyword(query: str) -> str:
        """从搜索 query 中提取「股票名 + 代码」作为精确关键词，降低泛化词干扰。"""
        code_match = re.search(r"\b(\d{6})\b", query or "")
        code = code_match.group(1) if code_match else ""
        zh_match = re.search(r"[\u4e00-\u9fff]{2,10}", query or "")
        name = zh_match.group(0) if zh_match else ""
...
class SearchService:
    ...
    def __init__(
        ...
        # 8. 东方财富免费资讯兜底（无需 Key；付费引擎额度耗尽/失败时兜底）
        self._providers.append(EastMoneySearchProvider())
        logger.info("已启用东方财富免费资讯兜底（无需 API Key）")
