...
class PortfolioRepository:
    ...
    # ------------------------------------------------------------------
    # 导出流水（portfolio_import_records）—— 完整记录持久化
    # ------------------------------------------------------------------
    def replace_import_records(self, account_id: int, records: List[Dict[str, Any]]) -> int:
        """重建某账户的导入流水（先清空再批量写入）。返回写入条数。

        ``records`` 元素字段：record_type / symbol / name / trade_date /
        trade_time / quantity / price / amount / fee / note / dedup_hash。
        """
        with self.portfolio_write_session() as session:
            session.execute(
                delete(PortfolioImportRecord).where(PortfolioImportRecord.account_id == account_id)
            )
            n = 0
            for r in records:
                session.add(
                    PortfolioImportRecord(
                        account_id=account_id,
                        record_type=str(r.get("record_type") or "其他"),
                        ...
                        note=str(r.get("note") or "")[:255] if r.get("note") else None,
                        dedup_hash=str(r.get("dedup_hash") or ""),
                    )
                )
                n += 1
            session.flush()
    ...
    def max_import_trade_date(self, account_id: int) -> Optional[date]:
        """返回该账户本地导入流水的最大成交日期（无记录返回 None）。"""
        with self.db.get_session() as session:
            row = session.execute(
                select(func.max(PortfolioImportRecord.trade_date)).where(
    ...
    def backup_import_records(self, account_id: int) -> int:
        ...
        with self.portfolio_write_session() as session:
            session.execute(
                delete(PortfolioImportRecordBackup).where(
                    PortfolioImportRecordBackup.account_id == account_id
                )
            )
            rows = session.execute(
                select(PortfolioImportRecord).where(PortfolioImportRecord.account_id == account_id)
            ).scalars().all()
            now = datetime.now()
            for r in rows:
                session.add(
    ...
    def restore_import_records(self, account_id: int) -> int:
        """用备份表恢复该账户导入流水（窗口覆盖的兜底回滚）。"""
        with self.portfolio_write_session() as session:
            session.execute(
                delete(PortfolioImportRecord).where(PortfolioImportRecord.account_id == account_id)
            )
            rows = session.execute(
                select(PortfolioImportRecordBackup).where(
                    PortfolioImportRecordBackup.account_id == account_id
                )
            ).scalars().all()
            for r in rows:
    ...
    def replace_import_records_window(
        self,
        account_id: int,
        records: List[Dict[str, Any]],
        window_start: date,
    ) -> Dict[str, Any]:
        """窗口覆盖导入流水：只删除/重写 ``window_start`` 及之后的记录，更早的历史保留。

        用于防止导出文件范围缩小（如只含今年）时误清历史：
        仅最近窗口与文件对齐，窗口外的本地历史原样保留。
        """
        # 仅保留文件里窗口内的记录（去重 by dedup_hash）
        window_records = [
            r for r in records if r["trade_date"] >= window_start
        ]
        seen: set = set()
        unique: List[Dict[str, Any]] = []
        for r in window_records:
            h = str(r.get("dedup_hash") or "")
            if h in seen:
                continue
            seen.add(h)
            unique.append(r)
        with self.portfolio_write_session() as session:
            session.execute(
                delete(PortfolioImportRecord).where(
                    PortfolioImportRecord.account_id == account_id,
                    PortfolioImportRecord.trade_date >= window_start,
                )
            )
            for r in unique:
                session.add(
    ...
    def list_import_records(
        ...
        start_date: date,
        end_date: date,
    ) -> List[PortfolioImportRecord]:
        with self.db.get_session() as session:
            rows = (
                session.execute(
                    select(PortfolioImportRecord)
                    .where(
                        PortfolioImportRecord.account_id == account_id,
                        PortfolioImportRecord.trade_date >= start_date,
                        PortfolioImportRecord.trade_date <= end_date,
                    )
                    .order_by(PortfolioImportRecord.trade_date, PortfolioImportRecord.id)
                )
                .scalars()
                .all()
            )
    ...
    def has_import_records(self, account_id: int) -> bool:
        with self.db.get_session() as session:
            row = session.execute(
                select(PortfolioImportRecord.id)
                .where(PortfolioImportRecord.account_id == account_id)
                .limit(1)
            ).first()
