...
def _price_tokens(value: object) -> list[str]:
    ...
    return re.findall(r"(?<![A-Za-z\d])(\d+(?:\.\d+)?)(?!\d|%|\s*日)", text)
...
def _compact_position(value: object, *, holding: bool) -> str:
    ...
    if holding and prices:
        parts = []
        hold_match = re.search(r"回踩不破\s*(\d+(?:\.\d+)?)\s*[^；;。]*?(?:可继续持有|持有|继续)", text)
        if hold_match:
            parts.append(f"回踩不破 {hold_match.group(1)} 可继续持有")
        stop_match = re.search(r"(?:放量)?跌破\s*(\d+(?:\.\d+)?)\s*[^；;。]*?(?:减仓|止损)", text)
        if stop_match:
            parts.append(f"跌破 {stop_match.group(1)} {'止损' if '止损' in stop_match.group(0) else '减仓'}")
        if parts:
            return "；".join(parts)
    if not holding:
        if "等待" in text or "企稳" in text:
            watch = re.search(r"回踩\s*(\d+(?:\.\d+)?)\s*附近企稳", text)
            if watch:
                return f"等回踩 {watch.group(1)} 附近企稳"
...
def _stock_data_from_payload(
    ...
    cci_val = _number_text(trend.get("cci"))
    cci_live_type = trend.get("cci_live_type")
    cci_signal_text = _clean_value(
        trend.get("cci_live_label") or trend.get("cci_signal_label"),
        limit=12,
    )
    cci_tone = (
        "positive"
        if cci_live_type == "buy"
        else "negative"
        if cci_live_type == "sell"
        else "neutral"
    )
    payload_technical = [
        item for item in (
            ...
            ("CCI", cci_val, "primary"),
            ("CCI信号", cci_signal_text, cci_tone),
            ("支撑", support, "positive"),
            ("压力", resistance, "negative"),
        ) if item[1]
    ]
...
def _footer(branding: ShareImageBranding, source_line: str, language: str) -> str:
    ...
    <footer class="poster-footer">
      <div class="{brand_class}">
        <div class="footer-title"><strong>SEA</strong><span>{_escape(PROJECT_DISPLAY_NAME)}</span></div>
...
<body>
  <main class="poster {report_kind}">
    <header class="poster-header"><div class="brand"><span class="brand-mark" aria-hidden="true"><i></i><i></i><i></i></span><strong>SEA</strong><em>|</em> {_escape(_poster_text(language, "brand"))}</div><div class="meta"><span class="date-chip">{_escape(stamp)}</span></div></header>
