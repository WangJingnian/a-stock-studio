...
class CciFitness(BaseModel):
    """CCI 指标历史回测适用性评估"""

    signals: Optional[int] = Field(None, description="回测信号总次数")
    wins: Optional[int] = Field(None, description="盈利信号次数")
    win_rate: Optional[float] = Field(None, description="胜率（%）")
    strategy_return: Optional[float] = Field(None, description="策略累计收益（%）")
    buy_hold_return: Optional[float] = Field(None, description="同期买入持有收益（%）")
    fitness: Optional[str] = Field(None, description="适用性结论：适用/一般/不适用/信号较少")
    summary: Optional[str] = Field(None, description="回测结论摘要")
    cci_now: Optional[float] = Field(None, description="最新 CCI 值")
...
class ReportStrategy(BaseModel):
    ...
    # CCI（顺势指标）点位
    cci: Optional[float] = Field(None, description="CCI(14) 当前值")
    cci_status: Optional[str] = Field(None, description="CCI 状态（超买/强势/弱势/超卖等）")
    cci_signal: Optional[str] = Field(None, description="CCI 信号描述")
    cci_cross_up: Optional[bool] = Field(None, description="是否上穿 +100")
    cci_cross_down: Optional[bool] = Field(None, description="是否跌破 +100")
    cci_cross_oversold: Optional[bool] = Field(None, description="是否上穿 -100")
    # CCI 直白信号（实时：基于最新两根 K 线穿越）
    cci_live_type: Optional[str] = Field(None, description="实时信号类型：buy/sell/watch")
    cci_live_label: Optional[str] = Field(None, description="实时信号标签：买入信号/卖出信号/观望")
    # CCI 确认信号（最近一次历史穿越，带日期与触发价）
    cci_signal_type: Optional[str] = Field(None, description="确认信号类型：buy/sell/watch")
    cci_signal_label: Optional[str] = Field(None, description="确认信号标签：买入信号/卖出信号/观望")
    cci_signal_date: Optional[str] = Field(None, description="最近一次确认信号日期（YYYY-MM-DD）")
    cci_signal_price: Optional[float] = Field(None, description="信号触发日收盘价（元）")
    cci_signal_trigger: Optional[str] = Field(None, description="信号触发原因描述")
    cci_fitness: Optional[CciFitness] = Field(None, description="CCI 历史回测适用性评估")
