...
def _extract_cci_from_record(result: Mapping[str, Any]) -> Mapping[str, Any]:
    """
    从历史记录的 raw_result.dashboard.data_perspective.trend_status 提取 CCI 指标字段。

    趋势分析引擎把 CCI 写入 dashboard（fill_cci_if_needed），这里把它附带到
    报告 strategy，供前端在“策略点位”下方展示 CCI 指标点位。
    """
    try:
        ...
        if not isinstance(trend_status, dict) or trend_status.get("cci") is None:
            return {}
        return {
            "cci": trend_status.get("cci"),
            "cci_status": trend_status.get("cci_status"),
            "cci_signal": trend_status.get("cci_signal"),
            "cci_cross_up": trend_status.get("cci_cross_up", False),
            "cci_cross_down": trend_status.get("cci_cross_down", False),
            "cci_cross_oversold": trend_status.get("cci_cross_oversold", False),
            "cci_signal_type": trend_status.get("cci_signal_type", "watch"),
            "cci_signal_label": trend_status.get("cci_signal_label", "观望"),
            "cci_signal_date": trend_status.get("cci_signal_date", ""),
            "cci_signal_price": trend_status.get("cci_signal_price", 0.0),
            "cci_signal_trigger": trend_status.get("cci_signal_trigger", ""),
            "cci_live_type": trend_status.get("cci_live_type", "watch"),
            "cci_live_label": trend_status.get("cci_live_label", "观望"),
            "cci_fitness": trend_status.get("cci_fitness"),
        }
...
def get_history_detail(
    ...
    try:
        ...
        strategy = ReportStrategy(
            ...
            **_extract_cci_from_record(result),
        )
