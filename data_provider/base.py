...
class DataFetcherManager:
    ...
    def get_belong_boards(self, stock_code: str) -> List[Dict[str, Any]]:
        ...
        # 兜底：东财 datacenter 概念/行业板块（免费接口，不依赖 push2，避免 boards 长期 failed）
        try:
            boards = self._fetch_belong_boards_em_datacenter(stock_code)
            if boards:
                return boards
        except Exception as exc:  # pragma: no cover - fail-open
            logger.debug("东财 datacenter 板块兜底失败 (%s): %s", stock_code, exc)
        return []

    @staticmethod
    def _fetch_belong_boards_em_datacenter(stock_code: str) -> List[Dict[str, Any]]:
        """东财 datacenter 核心题材板块（免费接口），作为 push2 不可用时的兜底。"""
        ...
        for row in rows:
            ...
            if not name or name in seen:
                continue
            seen.add(name)
            boards.append({"name": name, "type": "concept"})
        return boards
