...
const PortfolioPage: React.FC = () => {
  ...
  const [shareShowPnlAmount, setShareShowPnlAmount] = useState(true);
  const [shareShowPnlPct, setShareShowPnlPct] = useState(false);
  const [shareShowEquity, setShareShowEquity] = useState(false);
  ...
  const queryAccountId = selectedAccount == null || selectedAccount === 'all' ? undefined : selectedAccount;
  const refreshViewKey = `${selectedAccount == null || selectedAccount === 'all' ? 'all' : `account:${selectedAccount}`}:cost:${costMethod}`;
  const refreshContextRef = useRef<FxRefreshContext>({ viewKey: refreshViewKey, requestId: 0 });
  const hasAccounts = accounts.length > 0;
  const writableAccount = selectedAccount == null || selectedAccount === 'all' ? undefined : accounts.find((item) => item.id === selectedAccount);
  ...
  const loadAccounts = useCallback(async () => {
    try {
      ...
      setSelectedAccount((prev) => {
        ...
        if (prev == null) return items[0].id; // 首次加载默认选中第一个账户（百福具臻）
  ...
  // 盘中自动刷新：A股交易时段（周一~五 9:30-11:30 / 13:00-15:00）每 120 秒静默轮询一次
  // 最新快照（与后端 120s 行情缓存对齐），非交易时段不轮询，避免无效请求。
  const silentRefreshSnapshot = useCallback(async () => {
    try {
      const snapshotData = await portfolioApi.getSnapshot({
        accountId: queryAccountId,
        costMethod,
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      ...
      const isIntraday = (minutes >= 570 && minutes <= 690) || (minutes >= 780 && minutes <= 900);
      if (isIntraday) {
        void silentRefreshSnapshot();
      }
    }, 120000);
  ...
  const shIndex = useMemo(
    () => (snapshot?.indices || []).find((i) => String(i.code) === '000001'),
    [snapshot],
  );
  const szIndex = useMemo(
    () => (snapshot?.indices || []).find((i) => String(i.code) === '399001'),
    [snapshot],
  );
  ...
  const handleShareCard = async () => {
    ...
    setShareError(null);
    try {
      const blob = await portfolioApi.getShareImage({
        accountId: queryAccountId,
        costMethod,
        includeRealtime: true,
        showPnlAmount: shareShowPnlAmount,
        showPnlPct: shareShowPnlPct,
        showEquity: shareShowEquity,
      });
      const objectUrl = URL.createObjectURL(blob);
      const anchor = document.createElement('a');
      anchor.href = objectUrl;
      anchor.download = `持仓分享卡片-${getTodayIso()}.png`;
      ...
      const parsed = getParsedApiError(err);
      setShareError(parsed.message || text.shareCardFailed);
    } finally {
  ...
  return (
    <div className="portfolio-page min-h-screen space-y-4 p-4 md:p-6">
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.02] p-3">
            <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_220px_280px] gap-2 items-end">
              <div>
                ...
                <select
                  value={selectedAccount == null ? 'all' : String(selectedAccount)}
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  ...
                  disabled={shareLoading || !hasAccounts}
                  className="btn-secondary text-sm flex-1"
                >
                  {shareLoading ? text.sharingCard : text.shareCard}
                </button>
                <button
                  type="button"
      ...
      {shareError ? (
        <InlineAlert
          variant="warning"
          title={text.shareCard}
          message={shareError}
        />
      ) : null}
      {showShareDialog ? (
        <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4 space-y-3">
          <p className="text-sm font-semibold text-foreground">{text.shareDialogTitle}</p>
          <label className="flex items-center gap-2 text-sm text-secondary cursor-pointer">
            <input
              type="checkbox"
              checked={shareShowPnlAmount}
              onChange={(e) => setShareShowPnlAmount(e.target.checked)}
              className="h-4 w-4"
            />
            {text.shareShowPnlAmountLabel}
          </label>
          <label className="flex items-center gap-2 text-sm text-secondary cursor-pointer">
            <input
              type="checkbox"
              checked={shareShowPnlPct}
              onChange={(e) => setShareShowPnlPct(e.target.checked)}
              className="h-4 w-4"
            />
            {text.shareShowPnlPctLabel}
          </label>
          <label className="flex items-center gap-2 text-sm text-secondary cursor-pointer">
            <input
              type="checkbox"
              checked={shareShowEquity}
              onChange={(e) => setShareShowEquity(e.target.checked)}
              className="h-4 w-4"
            />
            {text.shareShowEquityLabel}
          </label>
          <p className="text-xs text-secondary">{text.sharePrivacyHint}</p>
          <div className="flex gap-2">
            ...
            >
              {shareLoading ? text.sharingCard : text.shareGenerate}
            </button>
          </div>
        </div>
      ) : null}
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
        <Card variant="gradient" padding="md">
          ...
          {isLoading && !snapshot ? (
            <div className="mt-2 h-7 w-28 animate-pulse rounded-md bg-white/10" aria-hidden="true" />
          ) : (
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
        <Card variant="gradient" padding="md">
          <p className="text-xs text-secondary">{text.dayPnl}</p>
          {isLoading && !snapshot ? (
            <div className="mt-2 h-7 w-28 animate-pulse rounded-md bg-white/10" aria-hidden="true" />
          ) : (
            <>
              <p className={`mt-1 text-xl font-semibold ${pnlColor(snapshot?.dayPnl)}`}>
                {formatMoney(snapshot?.dayPnl, snapshot?.currency || 'CNY')}
              </p>
              <p className={`text-xs ${pnlColor(snapshot?.dayPnlPct)}`}>{formatSignedPct(snapshot?.dayPnlPct)}</p>
            </>
          )}
        </Card>
        <Card variant="gradient" padding="md">
          <p className="text-xs text-secondary">{text.holdPnl}</p>
          {isLoading && !snapshot ? (
            <div className="mt-2 h-7 w-28 animate-pulse rounded-md bg-white/10" aria-hidden="true" />
          ) : (
            <>
              <p className={`mt-1 text-xl font-semibold ${pnlColor(snapshot?.holdPnl ?? snapshot?.unrealizedPnl)}`}>
                {formatMoney(snapshot?.holdPnl ?? snapshot?.unrealizedPnl, snapshot?.currency || 'CNY')}
              </p>
              <p className={`text-xs ${pnlColor(snapshot?.holdPnlPct ?? snapshot?.unrealizedPnlPct)}`}>
                {formatSignedPct(snapshot?.holdPnlPct ?? snapshot?.unrealizedPnlPct)}
              </p>
            </>
          )}
        </Card>
        <Card variant="gradient" padding="md">
          <p className="text-xs text-secondary">{text.shIndex}</p>
          {isLoading && !snapshot ? (
            <div className="mt-2 h-7 w-28 animate-pulse rounded-md bg-white/10" aria-hidden="true" />
          ) : (
            <>
              <p className="mt-1 text-xl font-semibold text-foreground">
                {formatIndexValue(shIndex?.value)}
              </p>
              <p className={`text-xs ${pnlColor(shIndex?.changePct)}`}>
                {shIndex ? formatSignedPct(shIndex.changePct) : '--'}
              </p>
            </>
          )}
        </Card>
        <Card variant="gradient" padding="md">
          <p className="text-xs text-secondary">{text.szIndex}</p>
          {isLoading && !snapshot ? (
            <div className="mt-2 h-7 w-28 animate-pulse rounded-md bg-white/10" aria-hidden="true" />
          ) : (
            <>
              <p className="mt-1 text-xl font-semibold text-foreground">
                {formatIndexValue(szIndex?.value)}
              </p>
              <p className={`text-xs ${pnlColor(szIndex?.changePct)}`}>
                {szIndex ? formatSignedPct(szIndex.changePct) : '--'}
              </p>
            </>
          )}
        </Card>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-[860px] w-full text-sm">
                <thead className="text-xs text-secondary border-b border-white/10">
                  <tr>
                    {showAccountColumn ? <th className="text-left py-2 pr-2">{text.account}</th> : null}
                ...
                <tbody>
                  {positionRows.map((row) => {
                    ...
                    <tr key={rowKey} className="border-b border-white/5">
                      {showAccountColumn ? <td className="py-2 pr-2 text-secondary">{row.accountName}</td> : null}
                      <td className="py-2 pr-2 font-mono text-foreground">{row.symbol}</td>
                      <td className="py-2 pr-2 text-foreground">{row.name || row.symbol}</td>
