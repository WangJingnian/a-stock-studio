...
import { Activity, BarChart3, Bell, BriefcaseBusiness, Gauge, Home, LineChart, LogOut, MessageSquareQuote, ReceiptText, RefreshCw, Search, Settings2 } from 'lucide-react';
...
const NAV_ITEMS: NavItem[] = [
  ...
  { key: 'statement', labelKey: 'layout.nav.statement', to: '/portfolio/statement', icon: ReceiptText },
  { key: 'equityCurve', labelKey: 'layout.nav.equityCurve', to: '/portfolio/equity-curve', icon: LineChart },
  { key: 'thsSync', labelKey: 'layout.nav.thsSync', to: '/portfolio/ths-sync', icon: RefreshCw },
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      >
        <div
          className={cn(
            'flex items-center justify-center overflow-hidden bg-primary-gradient text-[hsl(var(--primary-foreground))] shadow-[0_12px_28px_var(--nav-brand-shadow)]',
        ...
        >
          <img
            src="/sea-logo.jpg"
            alt="SEA"
            className={cn('h-full w-full object-cover', isRail ? '' : '')}
          />
        </div>
        {!collapsed ? (
          <p className={cn('min-w-0 truncate font-semibold text-foreground', isRail ? 'text-[0.95rem] leading-none' : 'text-sm')}>SEA</p>
        ) : null}
      </div>
