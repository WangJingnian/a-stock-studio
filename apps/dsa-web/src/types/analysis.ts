...
export interface ReportStrategy {
  ...
  cciSignal?: string;
  cciCrossUp?: boolean;
  cciCrossDown?: boolean;
  cciCrossOversold?: boolean;
  // CCI 直白信号（实时：基于最新两根 K 线穿越）
  cciLiveType?: 'buy' | 'sell' | 'watch';
  cciLiveLabel?: string;
  // CCI 确认信号（最近一次历史穿越，带日期与触发价）
  cciSignalType?: 'buy' | 'sell' | 'watch';
  cciSignalLabel?: string;
  cciSignalDate?: string;
  cciSignalPrice?: number;
  cciSignalTrigger?: string;
  // CCI 指标历史适用性回测
  cciFitness?: {
    signals?: number;
    wins?: number;
