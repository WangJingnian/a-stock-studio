...
function buildSnapshotParams(query: SnapshotQuery): Record<string, string | number> {
  ...
  if (query.showPnlAmount !== undefined) {
    params.show_pnl_amount = query.showPnlAmount ? 'true' : 'false';
  }
  if (query.showPnlPct !== undefined) {
    params.show_pnl_pct = query.showPnlPct ? 'true' : 'false';
  }
  if (query.showEquity !== undefined) {
    params.show_equity = query.showEquity ? 'true' : 'false';
  }
  return params;
}
...
export const portfolioApi = {
  ...
  async getShareImage(query: SnapshotQuery = {}): Promise<Blob> {
    const response = await apiClient.get<Blob>('/api/v1/portfolio/share-image', {
      params: buildSnapshotParams(query),
      responseType: 'blob',
    });
  ...
  async getMonthlyStatement(month: string, query: { accountId?: number; costMethod?: PortfolioCostMethod; useThs?: boolean } = {}): Promise<Record<string, unknown>> {
    const params: Record<string, string | number> = { month, cost_method: query.costMethod ?? 'fifo' };
    if (query.accountId != null) params.account_id = query.accountId;
    if (query.useThs != null) params.use_ths = query.useThs ? 'true' : 'false';
    const response = await apiClient.get<Record<string, unknown>>('/api/v1/portfolio/statement', { params });
  ...
  async getEquityCurve(query: { days?: number; accountId?: number; costMethod?: PortfolioCostMethod } = {}): Promise<Record<string, unknown>> {
    ...
    if (query.accountId != null) params.account_id = query.accountId;
    const response = await apiClient.get<Record<string, unknown>>('/api/v1/portfolio/equity-curve', { params });
    return toCamelCase(response.data);
  },
};
